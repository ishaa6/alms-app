...
const HolidaySchedule = () => {
  ...
  const {holiday} = useContext(UserContext);
  ...
  const renderCard = (item) => (
    ...
    >
      ...
      <Text className={`mt-2 font-bold ${getStatusColor(item.status)}`}>{item.status}</Text>
    </View>
  );

  useEffect(() => {
    const filtered = holiday.filter((leave) => {
      const matchesQuery = 
  ...
  }, [holiday, searchQuery, selectedDate, trigger]);
